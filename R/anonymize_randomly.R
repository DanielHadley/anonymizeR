...
#' @param write_key Writes a csv file in the working directory with each unique identifier and its corresponding ID. Defaults to FALSE.
...
anonymize_randomly <- function(mydf, identifier, id_length = 10, drop_old = TRUE, write_key = FALSE) {
  ...
  if(length(unique(byIdentifier$temp)) < length(byIdentifier$temp))
    warning('Two or more randomly assigned IDs were the same. Please set.seed() to a different number, and/or increase the length of the ID generated.')
  colnames(byIdentifier)[2] <- paste0(identifier, ".anon.ID")
  # Save a key in the working directory if TRUE
  if (write_key) {
    write.csv(byIdentifier,
              paste0("./anonymizer_key_", identifier, ".csv"),
              row.names=FALSE)
  }
